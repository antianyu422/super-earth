 ...
 * @private
 */
QuadtreePrimitive.prototype.render = function (frameState, camera, callback) {
  const passes = frameState.passes;
  ...
  if (passes.render) {
    ...
    if (camera) {
      // 根据虚拟相机选择需要渲染的瓦片
      const originCamera = frameState.camera
      const originCullingVolume = frameState.cullingVolume
      const originCommandList = frameState.commandList
      ...
      callback(frameState.commandList)

      frameState.camera = originCamera
      frameState.cullingVolume = originCullingVolume
      frameState.commandList = originCommandList
    }
