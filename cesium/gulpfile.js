...
function createTypeScriptDefinitions() {
  // Run jsdoc with tsd-jsdoc to generate an initial override.d.ts file.
  execSync("npx jsdoc --configure Tools/jsdoc/ts-conf.json", {
    stdio: "inherit",
  });

  let source = readFileSync("Source/override.d.ts").toString();
...
`;
  ...
  // Write the final source file back out
  writeFileSync("Source/override.d.ts", source);
